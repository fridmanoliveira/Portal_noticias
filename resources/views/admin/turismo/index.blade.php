                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($turismos as $turismo)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3 font-medium">{{ $turismo->titulo }}</td>
                                <td class="px-4 py-3 font-medium">
                                    {{ \Illuminate\Support\Str::limit(strip_tags($turismo->descricao), 50) }}
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-full
                                        {{ $turismo->ativo ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                        {{ $turismo->ativo ? 'Sim' : 'Não' }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <div class="flex items-center justify-center gap-3">
                                        ...
                                        <form method="POST" action="{{ route('admin.turismo.destroy', $turismo->id) }}"
                                              onsubmit="return confirm('Tem certeza que deseja excluir este turismo?')">
                                            @csrf
                                            @method('DELETE')
