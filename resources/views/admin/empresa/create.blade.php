<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-gray-800">{{ __('Criar Nova Empresa') }}</h2>
                <p class="mt-1 text-sm text-gray-500">Cadastre uma nova empresa no sistema</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-4xl px-4 mx-auto sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm rounded-xl">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('admin.empresa.store') }}" class="space-y-6">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <div class="mt-1">
                                    <input type="text" name="responsavel_legal" id="responsavel_legal" value="{{ old('responsavel_legal') }}" required
                                        class="block w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-[#0596A2] focus:border-[#0596A2] placeholder-gray-400">
                                </div>
                            </div>
