<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold leading-tight text-gray-800">
            {{ __('Editar Questão: ') . $question->title }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="p-8 bg-white shadow-xl rounded-2xl">
                <form action="{{ route('admin.questions.update', $question) }}" method="POST" class="space-y-6">
                    ...
                    <!-- Título -->
                    <div>
                        <label for="title" class="block mb-1 text-sm font-semibold text-gray-700">Título</label>
                    ...
                    <!-- Tipo -->
                    <div>
                        <label for="type" class="block mb-1 text-sm font-semibold text-gray-700">Tipo</label>
                    ...
                    <!-- Opções Dinâmicas -->
                    <div>
                        <label class="block mb-1 text-sm font-semibold text-gray-700">Opções</label>
                            ...
                            @foreach(old('options', $question->options->pluck('option_text')->toArray()) as $index => $opt)
                                <div class="flex gap-2">
                                    ...
                                    <button type="button" onclick="removeOption(this)"
                                        class="px-3 py-2 text-white bg-red-500 rounded-lg hover:bg-red-600">X</button>
                                </div>
                            @endforeach
                        </div>
