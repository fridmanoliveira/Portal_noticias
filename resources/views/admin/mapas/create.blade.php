<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-gray-800">{{ __('Cadastrar Nova Página de Mapas') }}</h2>
                <p class="mt-1 text-sm text-gray-500">Crie o conteúdo que será exibido na página pública de mapas</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-4xl px-4 mx-auto sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm rounded-xl">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('admin.mapas.store') }}" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        ...
                        <div>
                            <label for="arquivo_pdf" class="block text-sm font-medium text-gray-700">Arquivo PDF <span class="text-red-500">*</span></label>
                            <div class="mt-1">
                                 <input id="arquivo_pdf" name="arquivo_pdf" type="file" accept="application/pdf" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-teal-50 file:text-teal-700 hover:file:bg-teal-100" required>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="mt-1">
                                <input type="text" name="texto_botao" id="texto_botao" value="{{ old('texto_botao', 'Download do Arquivo') }}" required
                                    class="block w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-[#0596A2] focus:border-[#0596A2]">
                            </div>
                        </div>
