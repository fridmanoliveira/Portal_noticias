            ...
            <form method="GET" action="{{ route('obras.index') }}" class="mb-6">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full mt-1 border-gray-300 rounded-md shadow-sm">
                            ...
                            @foreach ($statusDisponiveis as $status)
                                <option value="{{ $status }}"
                                    {{ $filtros['status'] == $status ? 'selected' : '' }}>{{ $status }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="flex items-end">
                        <button type="submit"
                            class="w-full px-4 py-2 text-white bg-[#047a85] rounded-md hover:bg-[#0596A2]">
                            Filtrar
                        </button>
                    </div>
                </div>
                ...
                <div class="mt-4">
                    ...
                    <div class="flex mt-1">
                        <input type="text" name="search" value="{{ $filtros['search'] }}"
                            class="flex-1 border-gray-300 shadow-sm rounded-l-md" placeholder="Digite para buscar...">
                            ...
                            <tr class="transition hover:bg-gray-50">
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">
                                        {{ Str::limit($obra->descricao, 50) }}</div>
                                    ...
                                    @if ($obra->imagens->count() > 0)
                                        <div class="flex -space-x-2">
                                            ...
                                            @if ($obra->imagens->count() > 3)
                                                <span
                                                    class="flex items-center justify-center w-10 h-10 text-xs font-medium text-white bg-gray-500 border-2 border-white rounded-full">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @php
                                        // Pega o último andamento
                                        $ultimoAndamento = $obra->andamentos->last();
                                        $progresso = $ultimoAndamento->progresso ?? 0;
                                    ...
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div class="flex-1 h-3 overflow-hidden bg-gray-200 rounded-full">
                                            <div class="{{ $corProgresso }} h-3 rounded-full transition-all duration-500"
        ...
        <div class="px-4 mx-auto sm:container">
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                @foreach ($obras->take(3) as $obra)
                    <div
                        class="overflow-hidden transition transform bg-white rounded-lg shadow-md hover:shadow-lg hover:-translate-y-1">
                        <div class="relative h-48">
                            @if ($obra->imagens->isNotEmpty())
                                <img src="{{ asset($obra->imagens->first()->image_path) }}"
                                    alt="{{ $obra->descricao }}" class="object-cover w-full h-full">
                            @else
                                <img src="https://source.unsplash.com/random/600x400?construction,{{ $loop->index }}"
                                    alt="{{ $obra->descricao }}" class="object-cover w-full h-full">
                            @endif
                            <div
                                class="absolute bottom-0 left-0 right-0 p-2 text-sm font-semibold text-white bg-black bg-opacity-50">
                                {{ $obra->fonte_recurso }}
                            </div>
                        </div>
                        <div class="p-4">
                            <h3 class="mb-2 text-lg font-semibold text-gray-800">
                                {{ Str::limit($obra->descricao, 60) }}</h3>
