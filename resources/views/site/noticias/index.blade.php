    ...
    <section class="px-4 py-10 mx-auto font-sans sm:container">
        ...
        <div class="p-6 mb-8 bg-white rounded-lg shadow-md">
            ...
            <form action="{{ route('site.noticias.index') }}" method="GET"
                class="grid items-end grid-cols-1 gap-4 font-sans md:grid-cols-4">

                <div>
                    <label for="periodo_inicio" class="block mb-1 text-sm font-medium text-gray-700">Período Início</label>
                    <input type="date" name="periodo_inicio" slug="periodo_inicio"
                        value="{{ request('periodo_inicio') }}"
                        class="block w-full px-3 py-2 text-sm font-normal bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-cyan-500 focus:border-cyan-500">
                </div>

                <div>
                    <label for="periodo_fim" class="block mb-1 text-sm font-medium text-gray-700">Período Fim</label>
                    <input type="date" name="periodo_fim" slug="periodo_fim" value="{{ request('periodo_fim') }}"
                        class="block w-full px-3 py-2 text-sm font-normal bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-cyan-500 focus:border-cyan-500">
                </div>

                <div>
                    <label for="descricao" class="block mb-1 text-sm font-medium text-gray-700">Descrição</label>
                ...
                <div>
                    <label for="categoria_id" class="block mb-1 text-sm font-medium text-gray-700">Categoria</label>
                ...
                <div class="flex justify-start mt-4 space-x-4 col-span-full md:mt-0">
                    <button type="submit"
                        class="flex items-center px-4 py-2 text-sm font-semibold text-white bg-teal-600 rounded-md hover:bg-teal-700 focus:outline-none focus:shadow-outline">
                        <i class="mr-2 fas fa-search"></i> Pesquisar
                    </button>
                    <a href="{{ route('site.noticias.index') }}"
                        class="flex items-center px-4 py-2 text-sm font-semibold text-gray-800 bg-gray-300 rounded-md hover:bg-gray-400 focus:outline-none focus:shadow-outline">
        ...
        <div class="mb-4 text-sm text-gray-600">{{ $noticias->total() }} Notícia(s) encontrada(s).</div>

        <div class="grid grid-cols-1 gap-8 font-sans md:grid-cols-2 lg:grid-cols-3">
            @forelse ($noticias as $noticia)
                <div class="overflow-hidden transition-shadow duration-300 bg-white shadow-md rounded-xl hover:shadow-lg">
                    ...
                    <div class="p-5">
                        <h3 class="mb-1 text-lg font-semibold text-teal-800">
                            <a href="{{ route('site.noticias.show', $noticia->slug) }}"
                        ...
                        <div class="mt-4 text-right">
                            <a href="{{ route('site.noticias.show', $noticia->slug) }}"
                                class="text-sm font-medium text-teal-700 hover:text-teal-800">Leia mais »</a>
                        </div>
                    </div>
                </div>
            @empty
                <p class="text-sm text-center text-gray-600 col-span-full">Nenhuma notícia encontrada com os filtros aplicados.</p>
            @endforelse
        </div>
        ...
        @if ($noticias->hasPages())
            <div class="flex justify-end mt-8">
                <ul class="inline-flex items-center space-x-1 font-sans text-sm">
                    {{-- Anterior --}}
                    ...
                    @foreach ($noticias->links()->elements[0] ?? [] as $page => $url)
                        @if ($page == $noticias->currentPage())
                            <li class="px-3 py-1 text-white bg-teal-800 border border-teal-800 rounded">{{ $page }}</li>
                        @else
                            <li>
                                <a href="{{ $url }}"
